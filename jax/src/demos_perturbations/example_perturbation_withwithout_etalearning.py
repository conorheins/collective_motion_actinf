...
def run(init_key_num = 1, # number to initialize the jax random seed
    ...
    axes[0].set_title('No learning of $\eta$')
    axes[0].legend()

    alpha_all = init_dict['alpha'] * jnp.ones(N)
    eta0_all = jnp.ones(shape = (N,1,genmodel['ns_x']))
    ...
    def step_fn(carry, t):
        ...
        return (pos, vel, mu, preparams), (pos, vel, F, preparams['f_params_pre']['eta0'])
    _, history_perturb_learn = lax.scan(step_fn, init_state + (preparams,),  jnp.arange(n_timesteps_perturb))
    perturb_pos_i, perturb_vel_i, F_responses_learn, eta0_responses = history_perturb_learn[0], history_perturb_learn[1], history_perturb_learn[2], history_perturb_learn[3] # history of perturbed velocities and free energies
    ...
    axes[1].set_title('Learning of $\eta$ enabled')
    axes[1].legend()

    sector_idx = 0
    axes[2].plot(jnp.arange(0, len(genproc['t_axis'])), eta0_responses[:,:,0,sector_idx])
    axes[2].set_title(f'Beliefs about $\eta$ in sector {sector_idx}')
    plt.show()
