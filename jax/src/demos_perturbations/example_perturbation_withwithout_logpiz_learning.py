...
from genmodel import init_genmodel, compute_vfe_vectorized, create_temporal_precisions
...
Pi_z_temporal, _  = create_temporal_precisions(truncation_order=2, smoothness=1.0)
...
def run(init_key_num = 1, # number to initialize the jax random seed
    ...
    axes[0].set_title('No learning of $\log \pi_z$')
    axes[0].legend()

    logpiz_spatial_all = 0.5 * jnp.ones(shape=(N,genmodel['ns_phi'])) # sample a different vector of sector-wise sensory (log) spatial precisions for every agent

    def parameterize_Pi_z(logpi_z_spatial):
        """
        Parameterize Pi_z (sensory preicison matrix) with logpiz_spatial, which is a vector of sector-specific log precisions
        """
        Pi_z_spatial = jnp.diag(jnp.exp(logpi_z_spatial))
    ...
    def step_fn(carry, t):
        ...
        return (pos, vel, mu, preparams), (pos, vel, F, preparams['logpiz_spatial'])
    _, history_perturb_learn = lax.scan(step_fn, init_state + (preparams,),  jnp.arange(n_timesteps_perturb))
    perturb_pos_i, perturb_vel_i, F_responses_learn, logpiz_responses = history_perturb_learn[0], history_perturb_learn[1], history_perturb_learn[2], history_perturb_learn[3] # history of perturbed velocities and free energies
    ...
    axes[1].set_title('Learning of  $\log \pi_z$ enabled')
    axes[1].legend()

    sector_idx = 0
    axes[2].plot(jnp.arange(0, len(genproc['t_axis'])), logpiz_responses[:,:,sector_idx]) 
    axes[2].set_title(f'Beliefs about $\log \pi_z$ in sector {sector_idx}')
    plt.show()
