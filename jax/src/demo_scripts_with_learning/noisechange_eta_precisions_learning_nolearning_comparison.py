...
from genmodel import init_genmodel, parameterize_A0_no_coupling, create_temporal_precisions
...
eta_key, piw_key = random.split(new_key, 2)
...
def parameterize_Pi_w(logpi_w_spatial):
    """
    Parameterize Pi_w (process preicison matrix) with `s_w` under assumption of only two orders of motion
    """
    Pi_w_spatial = jnp.exp(logpi_w_spatial) * jnp.eye(genmodel['ns_x'])
    return jnp.kron(Pi_w_temporal, Pi_w_spatial)

preparams = {'f_params_pre': {'alpha': alpha_all, 
                              'eta0': eta0_all
                              },
             'logpiw_spatial': logpiw_spatial_all
            }
parameterization_mapping = {'f_params_pre': 
                                {'to_arg_name': 'f_params', 
                                'fn': parameterize_f_params
                                },
                            'logpiw_spatial': 
                                {'to_arg_name': 'Pi_w', 
                                'fn': parameterize_Pi_w
                                }    
                            }
