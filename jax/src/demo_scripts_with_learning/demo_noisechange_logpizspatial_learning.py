...
# axes[1].plot(jnp.arange(0, len(genproc['t_axis'])), logpizspatial_history[:,good_fish_ids]) # plot beliefs about first sector eta for all individuals over time

variance_history = 1. / jnp.exp(logpizspatial_history[:,good_fish_ids])

burn_in_time = 1000
# axes[1].plot(jnp.arange(0, len(genproc['t_axis'])),variance_history) # plot beliefs about first sector eta for all individuals over time
# axes[1].plot(jnp.arange(start = burn_in_time, stop = noise_change_t),variance_history[burn_in_time:noise_change_t], c = 'b', label = 'before noise change')
# axes[1].plot(jnp.arange(start = noise_change_t, stop = len(genproc['t_axis'])), variance_history[noise_change_t:], c = 'r', label = 'after noise change')

axes[1].plot(jnp.arange(start = burn_in_time, stop = noise_change_t),logpizspatial_history[burn_in_time:noise_change_t, good_fish_ids], c = 'b', label = 'before noise change')
axes[1].plot(jnp.arange(start = noise_change_t, stop = len(genproc['t_axis'])),logpizspatial_history[noise_change_t:, good_fish_ids], c = 'r', label = 'after noise change')
