...
pi_z_spatial_all = random.uniform(piz_key, minval = 0.95, maxval = 1.05, shape = (N,)) # sample a different sensory smoothness for every agent
...
def parameterize_Pi_z(Pi_z_preparams):
    ...
    Pi_z_spatial = pi_z_spatial * jnp.eye(genmodel['ns_phi'])
...
axes[1].plot(jnp.arange(0, len(genproc['t_axis'])), pizspatial_history) # plot beliefs about first sector eta for all individuals over time

sz_history = np.array(history[4])
axes[2].plot(jnp.arange(0, len(genproc['t_axis'])), sz_history) # plot beliefs about first sector eta for all individuals over time
