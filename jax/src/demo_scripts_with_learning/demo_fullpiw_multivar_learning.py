...
pi_w_spatial_all = random.uniform(piw_key, minval = 0.95, maxval = 1.05, shape = (N,genmodel['ns_x'])) # sample a different process precision for sector and every agent
...
def parameterize_Pi_w(Pi_w_preparams):
    ...
    Pi_w_spatial = jnp.diag(pi_w_spatial) # multivariate pi_w_spatial
...
_, sampling_key = random.split(piw_key)
particle_idx = random.choice(sampling_key, jnp.arange(N))

piwspatial_history = np.array(history[3])
axes[1].plot(jnp.arange(0, len(genproc['t_axis'])), piwspatial_history[:,particle_idx,:]) # plot beliefs about all sector pi_w for one individual over time

_, sampling_key = random.split(sampling_key)
sector_idx = random.choice(sampling_key, jnp.arange(genmodel['ns_x']))
axes[2].plot(jnp.arange(0, len(genproc['t_axis'])), piwspatial_history[:,:,sector_idx]) # plot beliefs about a particular sector's pi_w for all individuals over time

sw_history = np.array(history[4])
axes[3].plot(jnp.arange(0, len(genproc['t_axis'])), sw_history) # plot beliefs about s_z for all individuals over time
