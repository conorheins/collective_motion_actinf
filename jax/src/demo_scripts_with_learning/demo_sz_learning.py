...
learning_rates = 0.001 * np.ones(N)
learning_rates[0] = 0.0 # disable learning in one individual by setting the learning rate to 0
meta_params = initialize_meta_params(infer_lr = 0.1, 
                                    nsteps_infer = 1, 
                                    action_lr = 0.1, 
                                    nsteps_action = 1, 
                                    learning_lr = jnp.array(learning_rates), 
                                    nsteps_learning = 1, 
                                    normalize_v = True
                                    )
...
genmodel['Pi_z'] = vmap(parameterize_Pi_2do)(preparams['s_z'])
...
axes[1].plot(jnp.arange(0, len(genproc['t_axis'])), sz_history[:,0], c = 'r') # plot beliefs about first sector eta for all individuals over time
axes[1].plot(jnp.arange(0, len(genproc['t_axis'])), sz_history[:,1:], c = 'b') # plot beliefs about first sector eta for all individuals over time
